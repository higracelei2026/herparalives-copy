...
import type { CharacterCard, StoryPreferences, StoryPlan } from "@/lib/types";
...
export async function POST(request: Request) {
  ...
  if (llmConfigured()) {
    ...
    if (result.ok) {
      ...
      return NextResponse.json({ jobId: crypto.randomUUID(), status: "first_chapter_ready", chapters: result.data.plan.chapters, provider: "bailian", promptVersion: STORY_EDITOR_PROMPT_VERSION, story, plan: result.data.plan, preferences: body.preferences });
    }
  }
  ...
  return NextResponse.json({ jobId: crypto.randomUUID(), status: "first_chapter_ready", chapters: 5, provider: "safe-template", promptVersion: STORY_EDITOR_PROMPT_VERSION, story, plan, preferences: body.preferences });
}
